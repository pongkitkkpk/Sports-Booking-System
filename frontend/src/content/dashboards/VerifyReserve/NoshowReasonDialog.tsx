...
export default function NoShowReasonDialog({
  open,
  slot,
  slots = [],
  onClose,
  ...
  const titleText =
    mode === "noshow"
      ? isBulk
        ? `ยืนยันการ No Show (${slots.length} รายการ)`
        : "ยืนยันการ No Show"
      : isBulk
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      <DialogTitle sx={{ fontWeight: 600 }}>{titleText}</DialogTitle>
      ...
      <Formik
        ...
        onSubmit={async (values, { setSubmitting }) => {
          try {
            await Promise.resolve(onSubmit(values.reason.trim()));
            onClose();
          } catch (err) {
          ...
          <Form>
              ...
              {isBulk && (
                <Stack spacing={1} sx={{ mb: 2 }}>
                  <Typography variant="body2" color="text.secondary">
                    คุณเลือก {slots.length} รายการที่จะ{" "}
                    {mode === "noshow" ? "No Show" : "Reject"}
                  </Typography>
                  <List dense>
            ...
            <DialogActions>
              ...
              <LoadingButton
                type="submit"
                variant="contained"
                color={mode === "noshow" ? "warning" : "error"}
